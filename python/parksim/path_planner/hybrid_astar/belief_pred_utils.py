...
def occupancy_probability_multiple_spots_occ_dep(T, dynamic_veh_path_t, ped_path_t, Sigma_0, Sigma_0_ped, Q, center_spots, vac_spots, occ_spots_veh, occ_spots_ped, Car_obj):
    ...
    P_O_vacant = np.maximum(P_O_vacant, 0.0001)
    P_O_occ = np.maximum(P_O_occ, 0.0001)

    return P_O_vacant, P_O_occ

def occupancy_probability_multiple_spots_occ_dep_p(T, dynamic_veh_path_t, ped_path_t, Sigma_0, Sigma_0_ped, Q, center_spots, vac_spots, occ_spots_veh, occ_spots_ped, p_c, Car_obj):
    """
    Computes the recursive occupancy probability P(O_t) for multiple vehicles and multiple parking spots.
    ...
    P_O_vacant_result = np.zeros((T + 1, len(vac_spots)))  # Occupancy probability for each time step and parking spot
    P_O_occ_result = np.zeros((T + 1, len(occ_spots_veh) + len(occ_spots_ped)))  # Occupancy probability for each time step and parking spot, focussed on departing agent
    ...
    prob_t = prob_t_dist * (1 / (1 + np.exp(-0.001 * dot_prod / vel_uncertainty[np.newaxis, :])))  # including velocity information
    prob_t_new = 1 - np.prod(1 - prob_t, axis=1)  # Probability that at least one agent occupies each spot using only dynamic agents
    P_O_vacant_result[0] = prob_t_new[:len(vac_spots)]
    P_O_occ_result[0] = prob_t_new[len(vac_spots):len(vac_spots)+len(occ_spots_veh)+len(occ_spots_ped)]
    ...
    for k in range(1, T + 1):
        ...
        P_O_vacant[k] = alpha * P_O_vacant_result[k - 1] + prob_t_new[:len(vac_spots)] * (1 - P_O_vacant_result[k - 1])
        prior_k_vac = p_c[vac_spots]*(1-P_O_vacant[k]) + (1-p_c[vac_spots])*(P_O_vacant[k]) # for fully vacant spots (obs)
        P_O_vacant_result[k] = p_c[vac_spots]*P_O_vacant[k]/prior_k_vac

        # P_O_vacant[k] = prob_t_new[:len(vac_spots)]
        # P_O_occ[k] = prob_t_new[len(vac_spots):len(vac_spots)+len(occ_spots_veh)+len(occ_spots_ped)]

        P_O_occ[k] = 1-(alpha * (1-P_O_occ_result[k - 1]) + (1-prob_t_new[len(vac_spots):len(vac_spots)+len(occ_spots_veh)+len(occ_spots_ped)]) * (P_O_occ_result[k - 1]))
        prior_k_occ = np.hstack((p_c[occ_spots_veh], p_c[occ_spots_ped]))*P_O_occ[k] + (1-np.hstack((p_c[occ_spots_veh], p_c[occ_spots_ped])))*(1-P_O_occ[k]) # for fully occupied spots (obs)
        P_O_occ_result[k] = np.hstack((p_c[occ_spots_veh], p_c[occ_spots_ped]))*P_O_occ[k]/prior_k_occ

    P_O_vacant_result = np.maximum(P_O_vacant_result, 0.0001)
    P_O_occ_result = np.maximum(P_O_occ_result, 0.0001)

    return P_O_vacant_result, P_O_occ_result
