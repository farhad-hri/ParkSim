...
# from parksim.path_planner.hybrid_astar.belief_predict import map_lot
...
def get_occ_vac_spots(static_obs_kd_tree, center_spots, observed_spots, Car_obj):
    epsilon = 1.0
    dist_nearest_obst = static_obs_kd_tree.query(center_spots[observed_spots], p = np.inf,
                                                 distance_upper_bound=Car_obj.length / 2 + 0.1)[0]  # 0 is distance to nearest neighbor of each spot, 1 is corresponding obstacle index
    observed_spots_occ_ind = np.where(dist_nearest_obst <= Car_obj.width / 2 + 0.1)[0]
    occ_spots = sorted(observed_spots[observed_spots_occ_ind])
    vac_spots = sorted(list(set(observed_spots) - set(observed_spots[observed_spots_occ_ind])))
...
dynamic_veh_goal = np.array([np.hstack((center_spots[38] + np.array([-Car_obj.length/2-l_w/4, 2.0]), np.deg2rad(90.0))),
...
# Pedestrian
ped_0 = np.array([center_spots[28] + np.array([Car_obj.length/2, -Car_obj.width/2])
                  ])
...
obstacleX_dyn=copy.deepcopy(obstacleX)
obstacleY_dyn=copy.deepcopy(obstacleY)
t=0
T_pred = 5 # time steps
dynamic_veh_path_t = dynamic_veh_path[:, t:t+T_pred]
ped_path_t = ped_path[:, t:t+T_pred]
for veh_i in range(dynamic_veh_path_t.shape[0]):
    dyn_obst_x, dyn_obst_y = get_vertices_car(Car_obj, dynamic_veh_path_t[veh_i, 0, :])
    for i in range(len(dyn_obst_x)-1):
        line = np.linspace([dyn_obst_x[i], dyn_obst_y[i]], [dyn_obst_x[i+1], dyn_obst_y[i+1]], num=10, endpoint=False)
        obstacleX_dyn = obstacleX_dyn + line[:, 0].tolist()
        obstacleY_dyn = obstacleY_dyn + line[:, 1].tolist()

    # obstacleX_dyn = obstacleX_dyn + dyn_obst_x
    # obstacleY_dyn = obstacleY_dyn + dyn_obst_y

for ped_i in range(ped_path_t.shape[0]):
    obstacleX_dyn = obstacleX_dyn + [ped_path_t[ped_i, 0, 0]]
    obstacleY_dyn = obstacleY_dyn + [ped_path_t[ped_i, 0, 1]]

static_xy = np.array([obstacleX_dyn, obstacleY_dyn]).T
...
occ_spots, vac_spots = get_occ_vac_spots(static_obs_kd_tree, center_spots, observed_spots, Car_obj)
print(f"Occupied spots: {occ_spots}, Vacant Spots: {vac_spots}")

# Plotting the dynamic agents
for time_dynamic in range(length_preds):
    # plot vehicle
